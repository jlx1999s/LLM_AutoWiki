...
export function buildGraphExtractionPrompt(
  ...
  return [
    "You are an information extraction engine for a wiki ingestion pipeline.",
    "",
    LANGUAGE_RULE,
    "",
    `Source file: ${sourceFileName}`,
    "",
    "You MUST return one strict JSON object only. No markdown, no explanations, no code fences.",
    "",
    "Output schema:",
    "{",
    '  "source": {',
    '    "title": "string",',
    '    "summary": "string (4-10 sentences)",',
    '    "tags": ["string"],',
    '    "highlights": ["string"]',
    "  },",
    '  "nodes": [',
    ...
    '      "aliases": ["string"],',
    '      "summary": "string",',
    '      "evidence": ["short quote or fact from source"]',
    "    }",
    "  ],",
    '  "edges": [',
    "    {",
    '      "source": "node name",',
    '      "target": "node name",',
    '      "relation": "short snake_case relation",',
    '      "confidence": 0.0,',
    ...
    "",
    "Constraints:",
    "- Keep nodes high-signal only (typically 6-20 nodes).",
    "- Prefer concept for diseases/methods/principles/processes; prefer entity for organizations/people/agents/pathogens/drugs.",
    "- Every edge source/target must match a node name exactly.",
    "- relation must be one of: causes, treats, diagnoses_treats, complication_of, sequela_of, may_progress_to, associated_with.",
    "- confidence must be in [0,1].",
    "- Do not invent URLs.",
    "- If uncertain, reduce confidence instead of adding speculative edges.",
    "",
    purpose ? `## Wiki Purpose\n${purpose}` : "",
